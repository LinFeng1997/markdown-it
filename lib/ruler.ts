...
class Ruler<S extends State = State> {
  ...
  __compile__ () {
    ...
    chains.forEach(function (chain) {
      if(self.__cache__ ) self.__cache__[chain] = [];
      self.__rules__.forEach(function (rule) {
        if (!rule.enabled) { return; }

        if (chain && rule.alt.indexOf(chain) < 0) { return; }

        self.__cache__ && self.__cache__[chain].push(rule.fn);
      });
    });
  };
 ...
 * });
   * ```
   **/
  at(name: string, fn: MarkdownIt.Rule<S>, options?: any): void {
 ...
 * });
   * ```
   **/
  before(beforeName: string, ruleName: string, fn: MarkdownIt.Rule<S>, options?: any): void {
 ...
 * });
   * ```
   **/
  after(afterName: string, ruleName: string, fn: MarkdownIt.Rule<S>, options?: any): void {
 ...
 * });
   * ```
   **/
  push(ruleName: string, fn: MarkdownIt.Rule<S>, options?: any): void{
    var opt = options || {};
   ...
   **/
  getRules(chainName: string): MarkdownIt.Rule<S>[] {
    if (this.__cache__ === null) {
      this.__compile__();
      return this.__cache__ ? this.__cache__[chainName] : [];
    }
