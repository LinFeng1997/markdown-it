...
class ParserInline {
  ...
  //
  skipToken(state:State) {
    let ok: boolean | void,
      i: number,
      pos: number = state.pos,
      rules: MarkdownIt.Rule<State>[] = this.ruler.getRules(''),
      len: number = rules.length,
      maxNesting: number = state.md.options.maxNesting,
      cache = state.cache;
  ...
  parse(str: string, md: MarkdownIt, env: any, outTokens: Token[]): void {
    ...
    let rules = this.ruler2.getRules('');
  ...
  State = require('./rules_inline/state_inline');
}
